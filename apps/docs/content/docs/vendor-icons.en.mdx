...
Shumoku includes over 900 vendor icons hosted on CDN for optimal loading.
...
Icons are automatically loaded from `https://icons.shumoku.packof.me`. The renderer fetches icon dimensions to ensure proper aspect ratio rendering.

<Callout type="info">
Icons are cached and dimensions are resolved automatically. No manual configuration needed.
</Callout>
