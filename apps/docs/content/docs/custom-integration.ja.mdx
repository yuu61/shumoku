...
```typescript
import { HierarchicalLayoutEngine, SvgRenderer } from 'shumoku'

const engine = new HierarchicalLayoutEngine()
const layout = await engine.layout(networkGraph)
...
詳細な JSON 形式は [JSON スキーマ](/docs/json-reference) を参照してください。
...
<Cards>
  <Card title="JSON スキーマ" href="/docs/json-reference">
    詳細な JSON 形式
  </Card>
  <Card title="NetBox 連携" href="/docs/netbox">
    NetBox との連携
  </Card>
</Cards>
